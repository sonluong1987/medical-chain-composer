...
async function GrantAccessToPatient(tx) {  // eslint-disable-line no-unused-vars
    ...
    const practitionerRegistry = await getParticipantRegistry('org.acme.sample.Practitioner');
    ...
    if (patient.authorized) {
        patient.authorized.push(practitioner);
    }
    else {
        patient.authorized = [practitioner];
    }
    await practitionerRegistry.update(practitioner);
    await patientRegistry.update(patient);
}
